	...
	<div class="dataTablesContainer">
		...
		<div class="float-right">
			...
			<?php }?>
				...
				<!--PLY-HS00000002 - Hack para filtrar la exportación a Excel -->
				<?php
					if(!empty($_GET["fechaDesde"])) {
						$dateFrom = $_GET["fechaDesde"];
					} else {
						$dateFrom = date("Y-m-d");
					}
	
					if(!empty($_GET["fechaHasta"])) {
						$dateTo = $_GET["fechaHasta"];
					} else {
						$dateTo = date("Y-m-d");
					}
					...
					if(isset($dateFrom) && isset($dateTo)){
						...
						$url_exportar = $export_url.'/fechaDesde/'.$dateFrom.'/fechaHasta/'.$dateTo;
						$url_imprimir = $print_url.'/fechaDesde/'.$dateFrom.'/fechaHasta/'.$dateTo;
						if(isset($id_finca) && $id_finca > 0){
							$url_exportar .= '/finca/'. $id_finca;
							$url_imprimir .= '/finca/'. $id_finca;
						}
					} else {
						$url_exportar = $export_url;
						$url_imprimir = $print_url;
					}
					
					// if(isset($id_finca)){
					//     if($id_finca > 0) {
    				// 		$url_exportar = $export_url.'/fecha/'.$fecha_filtro.'/id_finca/'.$id_finca;
    				// 		$url_imprimir = $print_url.'/fecha/'.$fecha_filtro.'/id_finca/'.$id_finca;
					//     }
					// } else {
					// 	$url_exportar = $export_url;
					// 	$url_imprimir = $print_url;
				...
				<!--/PLY-HS00000002 - Hack para filtrar la exportación a Excel -->
					...
					<a role="button" class="add_button btn btn-dark" href="<?php echo $url_exportar?>">
						<i class="fa fa-download"></i>
						<?php echo $this->l('list_export');?>
					</a>
				<?php }?>
				<?php if(!$unset_export){?>
					<a role="button" class="add_button btn btn-dark" href="<?php echo $url_imprimir?>">
